...
public class HW5 {
	...
	public static void main(String[] args) throws IOException 
	{
		...
		// Convert data into a 2D numeric array
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		for (int i = 0; i < yTrue.length; ++i)
		{
			...
			// The instance was predicted successfully
			if (yTrue[i] == yPred[i])
			{
				correct++;
				if (yTrue[i] == Label.Positive)
				{
					truePositives++;
				}
			}
			// The instance was predicted unsuccessfully
			else
			{
				// Instance is in reality true
				if (yTrue[i] == Label.Positive)
				{
					falseNegatives++;
				}
				// Instance is in reality false
				else
				{
					falsePositives++;
				}
		...
		// Calculate scores
		double accuracy = correct / (double) yTrue.length;
